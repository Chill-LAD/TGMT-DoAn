"""
Script huấn luyện mô hình Ours v1 (Hybrid CNN-Frequency, KHÔNG có SE Attention).
...
def create_model(num_classes=2, backbone="resnet18", pretrained=True, dropout=0.5):
    """Tạo Ours v1 model (không có SE attention)."""
    from model_v1 import create_model_v1
    return create_model_v1(
...
def train(visible_train_dir=None, visible_val_dir=None,
         ...
         backbone="resnet18", checkpoint_dir="./checkpoints/ours_v1_combined", resume=None, merge=True):
    """
    Hàm huấn luyện chính cho Ours v1.
    """
    set_seed(Config.seed)

    device = torch.device(Config.device)
    print(f"Using device: {device}")
    print(f"Model: Ours v1 (Hybrid CNN-Frequency, NO SE Attention)")
...
def main():
    ...
    import argparse
    parser = argparse.ArgumentParser(description="Train Ours v1 Watermark Detection Model")
    ...
    parser.add_argument("--checkpoint_dir", type=str, default="./checkpoints/ours_v1_combined",
